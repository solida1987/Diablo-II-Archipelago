...
/* Skills that FIRE THE WEAPON rather than merely holding it.
 *
 * Authored from skills.txt itypea1-3: every class-owned skill whose required
 * item type is a launcher -- miss (bow/crossbow), jave, thro. Bow skills,
 * javelin throws, Double Throw. The passives that key off those weapons
 * (Critical Strike, Penetrate, Pierce) are in the data and so in the list;
 * keeping their requirement is vanilla and costs nothing.
 *
 * These are the one group whose weapon restriction must SURVIVE the blanket
 * removal below. The restriction is not a class-flavour rule D2 uses to say
 * no -- it is what stops the client from trying to fire an arrow out of a
 * staff. With it removed, a Sorceress whose random starting skills included
 * Fire Arrow could select and cast it while holding her staff; the client's
 * server-command queue desynchronised and D2 died on its own assert
 * (SCmd.cpp line 3910, ptCmd->bCmd == SCMD_WEAPONSWAP) -- or, depending on
 * where the stale queue pointer landed, an access violation. One player in
 * several would hit it, whenever the seed dealt a launcher skill into the
 * starting six.
 *
 * Cross-class these skills stay in the pool on purpose: anyone can equip a
 * plain bow, and with the restriction intact D2 itself refuses the cast
 * until one is worn -- the vanilla, well-tested path. */
static BOOL SkillFiresTheWeapon(int skillId) {
    static const short LAUNCHER_SKILL_IDS[] = {
        6, 7, 9, 11, 12, 15, 16, 20, 21, 22, 23,
        25, 26, 27, 31, 33, 35, 140,
    };
    for (int i = 0; i < (int)(sizeof(LAUNCHER_SKILL_IDS) / sizeof(short)); i++)
        if (LAUNCHER_SKILL_IDS[i] == skillId) return TRUE;
    return FALSE;
}
...
static void PatchAllSkillAnimations(void) {
    ...
    __try {
        ...
        int weaponPatched = 0, launcherKept = 0;
        int maxLvlPatched = 0;
        DWORD oldProt;
        ...
        for (int i = 0; i < cnt; i++) {
            ...
            /* ANIMATIONS ARE NOT TOUCHED HERE. See RestoreNativeAnimsForClass.
             *
             * This used to rewrite every class-specific animation (S1-S4, SQ,
             * TH, KK) to A1 for all seven classes at once, before anybody knew
             * which class was about to be played. RestoreNativeAnimsForClass
             * then put the player's OWN skills back to their real animations a
             * moment later, at character load.
             *
             * That write-and-unwrite killed the Sorceress: D2Client keeps a
             * queue of commands it has predicted and is waiting for the server
             * to confirm, and changing a skill's animation out from under it
             * desynchronised that queue. The game died on its own assert --
             * SCmd.cpp line 3910, ptCmd->bCmd == SCMD_WEAPONSWAP -- seconds
             * after entering town. She was hit because three of her skills
             * carry those animations natively; a class with none was never
             * rewritten and never crashed.
             *
             * RestoreNativeAnimsForClass computes the finished state for every
             * skill in one pass and knows the class, so it can do the whole job
             * without ever writing a value it has to take back. It is now the
             * only code that writes SKT_ANIM. The vanilla cache filled above is
             * what it reads. */

            /* FIX 2: Remove weapon type restrictions for class-owned skills
             * -- except the ones that fire the weapon (see above). */
            if (charClass >= 0 && charClass <= 6 && SkillFiresTheWeapon(i)) {
                launcherKept++;
            } else if (charClass >= 0 && charClass <= 6) {
                BOOL had = FALSE;
                for (int w = 0; w < 3; w++) {
        ...
        Log("SKILL PATCH (boot): %d weapon restrictions removed, %d launcher-weapon restrictions KEPT, %d maxlvl 20->99 (animations are applied per class at character load)\n",
            weaponPatched, launcherKept, maxLvlPatched);
    } __except(EXCEPTION_EXECUTE_HANDLER) {
...
void Skilltree_OnCharacterLoadHook(void) {
    ...
    /* The live player unit is not always up this early. Skipping the pass
     * used to be harmless because the boot patch had already applied a
     * blanket A1 fallback; now this is the only code that writes
     * animations, so a skip would leave every cross-class skill without
     * one. Fall back to the class the save file reported -- the same hint
     * InitSkillPool uses for exactly this window. */
    if (pc < 0 && g_skillClassHint >= 0) {
        pc = g_skillClassHint;
        Log("Skilltree_OnCharacterLoadHook: player unit not ready — using saved class %d\n", pc);
    }
    if (pc < 0) {
        Log("Skilltree_OnCharacterLoadHook: class unknown, skipping repatch\n");
        return;
    }
