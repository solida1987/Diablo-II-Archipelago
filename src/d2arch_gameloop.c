        ...
        if (pSrvReward) {
            ...
            __try {
                ...
                if (g_serverPendingGoldLevels > 0) {
                    ...
                    /* Announce HERE, not at receive time: this is the only place
                     * the amount exists (the receive side has no readable player
                     * level and used to print a hardcoded 0). */
                    {
                        char gmsg[64];
                ...
                if (Quests_PeekPendingDrop(&peekKind, &peekIdx) && g_cachedPGame && hD2Game) {
                    ...
                    if (nowMs - s_lastSpecDropMs < 250) {   /* was 1500ms; near-instant item delivery */
                        ...
                        if (peekKind == REWARD_DROP_BATCH) {
                            ...
                            /* deliver with the EXACT mechanism the Ctrl+V dev-menu item buttons use (the runes/gems/bases buttons the tester confirms work 100%). */
    ...
    __try {
        ...
        for (int hand = 0; hand < 2; hand++) {
            ...
            *last = id;
            ...
            /* PROVENANCE (the intermittent spawn crash, 22/8). A tester's
             * brand-new Sorceress sometimes SPAWNS with Fire Arrow on the
             * right hand instead of the staff's Fire Bolt -- no input, no
             * click -- and dies on D2's SCmd.cpp 3910 assert moments later.
             * Nothing in our logs said where that skill came from, because
             * this watch only reported wave-2 ids. Now every hand change is
             * logged, and the skill struct is probed for the unit that
             * GRANTS it: a dword that points at a unit whose type is 4 is
             * the granting ITEM (a staffmod), no such pointer means the
             * character actually knows the skill. That single line answers
             * whether the staff rolled the wrong skill or something taught
             * her one -- the two halves of the bug hunt. */
            {
                ...
                DWORD grantItem = 0, grantTxt = 0, grantOff = 0;
                if (pSk) {
                    for (DWORD off = 4; off <= 0x2C && !grantItem; off += 4) {
                        __try {
                            DWORD v = *(DWORD*)((DWORD)pSk + off);
                            if (v && (v & 3) == 0 && *(DWORD*)v == 4) {
                                grantItem = v;
                                grantTxt = *(DWORD*)(v + 0x04);   /* items.txt row */
                                grantOff = off;
                            }
                        } __except(1) {}
                    }
                ...
                if (grantItem)
                    Log("SKILLWATCH: %s hand -> id=%d tier=%d '%s' GRANTED BY ITEM row=%lu (unit=%08X at skill+0x%lX)\n",
                        hand ? "left" : "right", id, tier, nm, grantTxt, grantItem, grantOff);
                else
                    Log("SKILLWATCH: %s hand -> id=%d tier=%d '%s' (no granting item: the character KNOWS this skill)\n",
                        hand ? "left" : "right", id, tier, nm);
            }
            if (id < 357) continue;                              /* wave 2: extra detail below */
            const char* nm = "?"; int tier = 0;
            for (int i = 0; i < (int)SKILL_DB_COUNT; i++) {
                if (g_skillDB[i].id == id) { nm = g_skillDB[i].name; tier = g_skillDB[i].tier; break; }
            }
