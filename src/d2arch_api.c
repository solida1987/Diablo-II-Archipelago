...
/* NOTE: there is no D2Client "SendGamePacket" pointer any more.
 *
 * This used to hold D2Client + 0x143E0 and call it with inline asm. That
 * address is NOT a send function -- it is D2Client's weapon-swap command
 * handler, which opens with
 *
 *     cmp byte ptr [ecx], 97h        ; 97h = SCMD_WEAPONSWAP
 *     ...
 *     push 0F46h                     ; 3910
 *     push "...D2Client\GAME\SCmd.cpp"
 *     push "ptCmd->bCmd == SCMD_WEAPONSWAP"
 *
 * The asm block set EDI and pushed the packet, but never ECX, so every raw
 * packet we sent called that handler with whatever happened to be in ECX.
 * Three outcomes, by luck alone: garbage that read as 97h walked on into the
 * swap handler, garbage that read as anything else raised D2's assert box,
 * and an unmapped pointer raised an access violation. That is the whole of
 * the "sometimes it crashes on startup" report -- and the reason SKILLGUARD
 * never appeared in a log is that its Log() call sits after the send, so the
 * process died before the line was written.
 *
 * D2Net ordinal 10005 is the real client->server send, already resolved as
 * fnClientSend and already used by SendSpendSkillPacket. Everything goes
 * through it now. */
...
static void __declspec(noinline) SendGamePacket(DWORD len, BYTE* packet) {
    if (!fnClientSend || !packet || len == 0) return;
    ...
    __try {
        fnClientSend(0, (const unsigned char*)packet, (int)len);
    } __except(EXCEPTION_EXECUTE_HANDLER) {
        Log("SendGamePacket: exception sending [0x%02X] len=%lu\n",
            packet[0], len);
    }
}
