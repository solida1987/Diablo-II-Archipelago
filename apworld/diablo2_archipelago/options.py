...
class ShopShuffle(Toggle):
    """Every vendor draws its shelf from the WHOLE item list on all three
    tabs: weapons, armor, gems, runes, jewels, charms, rings and amulets, as
    normal, magic, rare, set and unique items. Priced by the game itself, so
    the best pieces cost a fortune. The shelf re-rolls every time the vendor
    restocks. Consumables (potions/scrolls/keys/ammo) stay as in vanilla.
    Items you BUY count for the Collection book and its checks. Quest items
    never appear. Applied by the launcher via the data files + the mod."""
